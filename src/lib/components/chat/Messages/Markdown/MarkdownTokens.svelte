<script lang="ts">
	...
	const getHeaderClasses = (depth: number) => {
		const baseClasses = 'font-bold mb-2 mt-4';
		switch (depth) {
			case 1:
...
{#each tokens as token, tokenIdx (tokenIdx)}
	...
	{:else if token.type === 'heading'}
		<svelte:element this={headerComponent(token.depth)} class={getHeaderClasses(token.depth)} dir="auto">
	...
	{:else if token.type === 'blockquote'}
		...
		{:else}
			<blockquote dir="auto" class="border-l-4 border-gray-300 pl-4 py-2 mb-4 italic bg-gray-50 dark:bg-gray-800 dark:border-gray-600">
				<svelte:self id={`${id}-${tokenIdx}`} tokens={token.tokens} {onTaskClick} {onSourceClick} />
			</blockquote>
		{/if}
	{:else if token.type === 'list'}
		{#if token.ordered}
			<ol start={token.start || 1} dir="auto" class="list-decimal list-inside mb-4 ml-4 space-y-1">
		...
		{:else}
			<ul dir="auto" class="list-disc list-inside mb-4 ml-4 space-y-1">
				{#each token.items as item, itemIdx}
	...
	{:else if token.type === 'paragraph'}
		<p dir="auto" class="mb-4 leading-relaxed">
			<MarkdownInlineTokens
				id={`${id}-${tokenIdx}-p`}
				tokens={token.tokens ?? []}
				{onSourceClick}
